...
function CarouselOfThree() {
  ...
  const animalItemsThree = sheltered?.map((animal, index) => (
    <div
      key={animal.id}
      className={'carousel3-img place-self-center ' + (index < 3 ? '' : 'hidden')}
    >
      <div className="flex bg-zoning rounded-lg shadow dark:bg-gray-800 flex-row md:flex-col p-4">
        <div className="w-full md:w-full flex justify-center items-center">
          {animal.images_animal ? (
            <img
              className="object-contain w-[80%] h-48 md:h-full rounded-lg"
        ...
        <div className="flex-auto text-center">
          <div className="flex flex-wrap my-2">
            ...
            <hr />
            <p className="flex-none w-full mt-2 text-sm font-medium text-gray-500 dark:text-gray-300">
              Age : {animal.age}
            </p>
            <p className="flex-none w-full mt-2 text-sm font-medium text-gray-500 dark:text-gray-300">
          ...
          <div className="flex mb-4 text-sm font-medium">
            <Link
              className="py-2 px-4 bg-accents1-light text-fond w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-lg"
  ...
  return (
    ...
    >
      <button
        ...
        tabIndex={0}
      >
        <img src="/icons/left.svg" alt="" onClick={getThreePreviousPic} />
      </button>

      <div className="h-auto w-auto flex rounded-lg my-6 gap-4 px-8">{animalItemsThree}</div>
      ...
      <button
        ...
        tabIndex={0}
      >
        <img src="/icons/right.svg" alt="" onClick={getThreeNextPic} />
      </button>
    </section>
  );
}
