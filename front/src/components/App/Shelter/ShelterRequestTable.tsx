    ...
    <tbody>
      ...
      >
        <td
          colSpan={4}
          scope="colgroup"
          className="px-2 pt-2 border-accents2-dark border-solid border-1"
        >
          {animal.nom}
        </td>
        <td
      ...
      <tr className="fold text-fond text-sm bg-accents2-light font-grands font-semibold p-3 border-accents2-dark border-solid border-1 hidden">
        <td colSpan={2} className="px-2 pt-2  border-accents2-light border-solid border-1">
          Famille
        </td>
        <td colSpan={2} className="px-2 pt-2  border-accents2-light border-solid border-1">
          Date de demande
      ...
      {animal.demandes.map((demande: typeof animal.demandes, index: number) => (
        <tr
          key={`${animal.id} request n° ${demande.id}`}
          className={
            'fold text-sm font-body font-semibold hidden ' +
            (index % 2 > 0 ? 'text-fond bg-accents2-light' : 'bg bg-fond')
          }
        >
          ...
          <td colSpan={2}>
            <Link
              ...
              to={`/associations/profil/demandes/${demande.Demande.id}`}
            >
