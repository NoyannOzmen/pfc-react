...
function ShelterResidentDetails() {
  ...
  const tagItems = animal.tags.map((tag: ITag) => (
    <p
      key={tag.id}
      className="group rounded-full block bg-accents1 text-fond text-center text-xs font-semibold py-1 px-2"
    >
      {tag.nom}
  ...
  const requestItems = animal.demandes.map((demande: typeof animal.demandes) => (
    <tr
      key={demande.id}
      className="odd:bg-accents2-light even:bg-fond odd:text-fond text-sm font-body font-semibold p-4 rounded-lg "
    >
      ...
      <td className="text-center p-2 rounded-lg hover:underline">
        <Link to={`/associations/profil/demandes/${demande.Demande.id}`}>
          {demande.Demande.statut_demande}
        </Link>
      </td>
    </tr>
  ));
  ...
  async function sendFile(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (file) {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/upload/photo`, {
          method: 'POST',
      ...
      <div className="flex flex-col content-center justify-center mx-auto mb-4 w-[80%]">
        ...
        <div className="flex flex-col bg-zoning rounded-lg">
          ...
          <section
            ...
            className="flex justify-center flex-wrap gap-x-6 gap-y-4 p-6"
          >
            {userMessage && (
              <div>
            ...
            <div className="w-60 md:w-auto">
              ...
              {animal.tags && <div className="flex flex-wrap mt-4 px-6 gap-1">{tagItems}</div>}
              ...
              <div className="px-4">
                ...
                <table className="mb-3 rounded-b-lg rounded-lg border-separate ">
                  <thead className=" text-fond text-sm bg-accents2 font-grands font-semibold p-3 border-accents2-dark border-solid border-1">
                    <tr>
                      <th className="px-2 pt-2  border-accents2-light border-solid border-1 text-center">
                        Famille
                      </th>
                      <th className="px-2 pt-2  border-accents2-light border-solid border-1 text-center">
                        Date de demande
                  ...
                  <tbody className="rounded-lg border-separate ">{requestItems}</tbody>
                </table>
              </div>
            ) : (
              <></>
