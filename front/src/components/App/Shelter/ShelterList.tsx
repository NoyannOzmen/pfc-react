...
function ShelterList() {
  ...
  const [searchedShelters, setSearchedShelters] = useState(available);
  ...
  const animalSelect: NodeListOf<HTMLInputElement> =
    document.querySelectorAll('input[name="espece"]');
  ...
  function filterResultsFull(event: React.MouseEvent<HTMLElement>) {
    ...
    if (espece.length > 0) {
      let filteringArray: IAssociation[] = [];
    ...
    <main className="justify-self-stretch flex-1">
      ...
      <div className="md:my-3 flex flex-wrap font-body w-full bg-zoning rounded-lg shadow dark:bg-gray-800 justify-around">
        <form className="text-texte justify-around">
          ...
          >
            <h2 className="font-grands text-2xl w-full my-2 text-center">
            ...
            <input
              ...
              className="w-1/3 col-span-1 mx-auto my-3 py-2 px-2 bg-accents1-light text-fond transition ease-in duration-200 text-center text-xs font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-lg"
              type="button"
              value="Rechercher"
            />
      ...
      <div className="flex flex-wrap content-center justify-around my-8">
        <section className="mx-auto w-[80%]">
          ...
          <p className="mx-auto text-l font-body text-center">
      ...
      {searchedShelters.length ? (
        <div className="grid grid-cols-3 gap-3 m-3">{shelterItems}</div>
      ) : (
