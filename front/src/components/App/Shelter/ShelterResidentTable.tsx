  ...
  return (
    <tbody>
      <tr
        onClick={handleClick}
        tabIndex={0}
        className="view  text-fond text-sm bg-accents2 font-grands font-semibold p-3 border-accents2-dark border-solid border-1 hover:bg-accents2-dark"
      >
        <td className="px-2 pt-2  border-accents2-dark border-solid border-1">{animal.nom}</td>
        <td className="px-2 pt-2  border-accents2-dark border-solid border-1">
            ...
            <div className="w-full md:w-1/2">
              ...
              <div className="flex p-6 pb-4">
                <div className="flex flex-col gap-2">
                  ...
                  <Link
                    className="rounded-full block bg-accents1 text-fond w-28 text-center text-xs font-semibold px-6 hover:underline"
              ...
              {animal.tags && (
                <div className="flex flex-wrap mt-4 px-6 gap-1">
                  {animal.tags.map((tag: ITag) => (
                    <p
                      key={`${animal.id} tag n° ${tag.id}`}
                      className="group p-2 rounded-full block justify-center bg-accents1-light text-fond text-center text-sm font-semibold py-1 px-2"
                    >
                      {tag.nom}
                      <span className="group-hover:block hidden z-10 bg-accents2 text-fond absolute px-2 py-2 text-sm rounded-b-xl rounded-tr-xl">
                        {tag.description}
                      </span>
                    </p>
                  ))}
