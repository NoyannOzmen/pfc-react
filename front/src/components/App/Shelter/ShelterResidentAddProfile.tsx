...
function ShelterResidentAddProfile() {
  ...
  const speciesItems = species.map(espece => (
    <option key={`Espece ${espece.id}`} value={espece.id}>
      {espece.nom}
    </option>
  ));
  ...
  const tagItems = tags.map(tag => (
    <div key={`Tag ${tag.id}`} className="flex gap-x-1.5 w-full">
      <input
        ...
        name={tag.id}
        value={tag.id}
        className="leading-3 size-6"
      />
      <label htmlFor={tag.id} className="block font-grands text-xs leading-3">
        {tag.nom}
      </label>
    </div>
  ));
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/animaux/nouveau-profil`, {
        method: 'POST',
        headers: {
  ...
  function handleCreateTag(event: React.FormEvent<HTMLFormElement>) {
    ...
    async function createTag() {
      setUserMessage(null);
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/tags/create`, {
          method: 'POST',
          headers: {
            'Content-type': 'application/json',
            Authorization: `Bearer ${token}`,
      ...
      <div className="flex flex-col content-center justify-center mx-auto mb-4 w-[80%]">
        ...
        <div className="flex flex-col bg-zoning rounded-lg">
          ...
          <section className="flex flex-col justify-center content-center">
            ...
            <form
              className="grid grid-cols-1 my-6 mx-6 justify-center lg:flex-none lg:mx-2 lg:grid lg:grid-cols-3 lg:px-2 xl:grid-cols-3 xl:p-10"
            ...
            >
              <fieldset className="flex flex-wrap justify-center content-start gap-x-4">
                <div className="mb-2 w-[90%]">
                  <label
                    htmlFor="nom_animal"
                    className="block font-grands w-full font-bold text-base "
                  >
                    Nom
                  </label>
                  <input
                    className="w-full rounded-md h-8 px-2 py-1 text-texte bg-fond "
                    type="text"
                    name="nom_animal"
                    id="nom_animal"
                    required
                ...
                <div className="mb-2 w-[90%]">
                  <label
                    htmlFor="sexe_animal"
                    className="block w-full font-grands font-bold text-base "
                  >
                    Sexe
                  </label>
                  <select
                    name="sexe_animal"
                    id="sexe_animal"
                    className="custom-select w-full rounded-md h-8 px-2 py-1 text-texte bg-fond "
                  >
              ...
              <fieldset className="flex flex-wrap justify-center gap-x-4">
                <div className="mb-2 grow ">
                  <label
                    htmlFor="description_animal"
                    className="block font-grands font-bold text-base w-full"
                  >
                    Description
                  </label>
                  <textarea
                    className="w-full rounded-md px-2 py-1 text-texte bg-fond "
                ...
                <div className="mb-2 w-full">
                  <p
                    id="couleur-animal"
                    className="block font-grands font-bold text-base mb-4 shrink"
                  >
                    Tags
                  </p>
                  ...
                  <fieldset
                    name="tags-animal"
                    id="tags-animal"
                    className="grid grid-cols-1 md:grid-cols-2 gap-2 w-full px-2 py-1"
                  >
                    {tagItems}
                  </fieldset>
                  ...
                  <div className="flex justify-center">
                    <button
                      ...
                      type="button"
                      id="create-tag"
                      className="self-center hover:bg-accents1-dark rounded-full hover:underline bg-accents1-light text-center font-grands text-fond font-semibold text-base py-0.5 px-4"
                    >
                      Créer un tag
                    </button>
                  </div>
              ...
              <fieldset className="flex flex-wrap justify-center w-full lg:col-start-2 lg:columns-2">
                <div className="">
                  <input
                    ...
                    className="hover:bg-accents1-dark rounded-full hover:underline bg-accents1 text-center font-grands text-fond font-semibold text-xl py-2 px-6"
                  />
                </div>
              </fieldset>
            </form>
        ...
        <div className="self-center bg-zoning p-6 rounded-lg">
          ...
          <form id="create-tags-form" className="" onSubmit={handleCreateTag}>
            <div className="mb-2">
              <label
                htmlFor="tag-name"
                className="block text-texte font-grands font-bold text-base "
              >
                Nom du Tag
              </label>
              <input
                className="w-56 rounded-md h-8 px-2 py-1 text-texte bg-fond "
                type="text"
                name="tag_name"
                id="tag-name"
                required
            ...
            <div className="mb-4 ">
              ...
              <textarea
                className="w-56 rounded-md px-2 py-1 text-texte bg-fond"
