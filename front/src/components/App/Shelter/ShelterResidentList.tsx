...
function ShelterResidentList() {
  ...
  const sheltered = animals.filter(
    ({ association_id }) => Number(association_id) === Number(auth.user?.id)
  );
  ...
  const shelteredItems = sheltered.map(animal => (
    <Link
      ...
      className="animal_card animal_card--visible flex flex-col justify-between content-center relative bg-fond rounded-xl w-36 h-36 shrink-0 md:size-60"
      to={`/associations/profil/animaux/${animal.id}`}
    >
      {animal.images_animal[0].url ? (
        <img
          className="rounded-t-xl"
          src={`${import.meta.env.VITE_API_URL}` + `${animal.images_animal[0].url}`}
  ...
  const speciesItems = species.map(espece => (
    <div key={espece.id} className="flex gap-x-1.5 content-center mb-1">
      <input
        ...
        type="checkbox"
        key={espece.id}
        id={`espece_${espece.id}`}
        name={`espece_${espece.id}`}
        value={`espece_${espece.id}`}
        className="species-checkbox checkbox leading-3"
      />
      <label
        htmlFor={`espece_${espece.id}`}
        className="font-grands font-semibold text-xs leading-3 self-center"
      >
        {espece.nom}
      </label>
    </div>
      ...
      <div className="flex flex-col content-center justify-center mx-auto mb-4 w-[80%]">
        ...
        <div className="flex flex-col bg-zoning rounded-lg relative">
          ...
          <section id="dahboard-container" className="flex flex-col justify-center content-center">
            ...
            <h3 className="text-center hidden md:inline font-grands text-4xl font-extrabold mt-4">
              Animaux
            </h3>
            ...
            <form
              autoComplete="off"
              className="my-4 px-4 flex flex-wrap gap-3 justify-center md:w-1/4 md:absolute md:top-0 md:right-5 md:my-2 md:p-0 md:pr-4 md:justify-end z-10"
            ...
            >
              <div key={'shelteredSearch'} className=" flex gap-x-1.5 text-center h-5">
                ...
                <input
                  ...
                  id="search-bar"
                  className="bg-fond rounded-full block pl-2 md:w-32 lg:w-full shrink-0"
                  type="text"
                ...
                <span
                  ...
                  role="button"
                  className="material-symbols-outlined bg-fond rounded-full"
                >
                  arrow_drop_down
                </span>
              </div>
              ...
              <div
                key={'shelteredFilter'}
                id="search-filters"
                className="gap-4 hidden md:bg-fond p-4 rounded-lg border-accents2 md:border-4"
              >
                <fieldset key={'first'}>
                  {speciesItems}
                ...
                <fieldset key={'second'}>
                  <div className="flex gap-x-1.5 content-center">
                    <input
                      ...
                      value="En refuge"
                      className="mb-1 statut-checkbox checkbox leading-3"
                    />
                    <label
                      htmlFor="satut_En_refuge"
                      className=" font-grands font-semibold text-xs leading-3 self-center"
                    >
                      En refuge
                    </label>
